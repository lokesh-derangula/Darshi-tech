...
import { Film, Radio, Layers, FileText, HelpCircle, CreditCard, Sparkles } from 'lucide-react';

export default function JeeEapcetTraining() {
  ...
  const features = [
    { title: 'Recorded Lectures', desc: 'Pre-recorded video libraries covering advanced calculus, quantum theory parameters, and organic chemistry mechanisms.', icon: Film, color: 'text-[var(--text-serif)] bg-[var(--pill-bg)] border-[var(--glass-border)] transition-colors duration-300' },
    { title: 'Live Classes', desc: 'Active interactive classes, problem-solving marathons, and board exam reviews with subject experts.', icon: Radio, color: 'text-[var(--text-serif)] bg-[var(--pill-bg)] border-[var(--glass-border)] transition-colors duration-300' },
    { title: 'Mock Tests', desc: 'Custom timing exams, performance analytics, speed reviews, and comprehensive solutions worksheets.', icon: Layers, color: 'text-[var(--text-serif)] bg-[var(--pill-bg)] border-[var(--glass-border)] transition-colors duration-300' },
    { title: 'Question Banks', desc: 'Access over 10,000+ targeted practice questions, formulas lists, cheatsheets, and quick guides.', icon: FileText, color: 'text-[var(--text-serif)] bg-[var(--pill-bg)] border-[var(--glass-border)] transition-colors duration-300' },
    { title: 'Doubt Sessions', desc: '24/7 dedicated discussion forums, personalized chats, and daily doubt clearance channels.', icon: HelpCircle, color: 'text-[var(--text-serif)] bg-[var(--pill-bg)] border-[var(--glass-border)] transition-colors duration-300' }
  ];
  ...
  return (
    <div className="space-y-12 py-4">
      <div className="space-y-2">
        <h1 className="font-serif text-3xl md:text-4xl font-light text-theme-title">JEE Mains & EAPCET Prep Labs</h1>
        <p className="text-theme-muted text-base uppercase tracking-wider">Engineering entrance examination syllabus and question databases</p>
      </div>

      {/* Preparation Features Grid */}
        ...
        {features.map((feat, i) => {
          ...
          return (
            <div key={i} className="glass-panel p-6 flex flex-col justify-between transition-all duration-300">
              <div>
                <div className={`inline-flex p-2.5 border mb-4 rounded-xl ${feat.color}`}>
                  <Icon className="h-4 w-4" />
                </div>
                <h3 className="font-serif text-lg font-bold text-theme-card-title mb-2">{feat.title}</h3>
                <p className="text-theme-desc text-base leading-relaxed">{feat.desc}</p>
              </div>
            </div>
          );
        })}
      ...
      <section className="space-y-6 pt-6 border-t border-theme-divider">
        <div>
          <h2 className="font-serif text-2xl font-light text-theme-title">JEE & EAPCET Course Programs</h2>
          <p className="text-theme-muted text-base uppercase tracking-wider">Select our structured training bootcamps</p>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-10 text-theme-muted text-base font-semibold">No entrance prep courses currently available.</div>
        ) : (
